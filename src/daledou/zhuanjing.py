...
class ZhuanJing(DaLeDou):
    ...
    def biaoxingtianxia(self) -> int:
        ...
        jifen: list = DaLeDou.findall(r'我的威望：(\d+)')
    ...
    def beibao(self):
        ...
        for index, id in enumerate(list):
            ...
            number: list = DaLeDou.findall(r'数量：(\d+)')
            if number:
    ...
    def run(self) -> list:
        if self.祝福合集宝库():
            self.msg += DaLeDou.conversion('专精')
            self.msg += ['升级武器专精所需投掷、小型、中型、大型武器符文石数量']
            self.msg += ['需要  拥有（商店+背包）  失败祝福值']
