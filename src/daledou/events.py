...
class EventsTwo(DaLeDou):
    ...
    def 神魔转盘(self):
        EventsTwo.get('cmd=newAct&subtype=88&op=1')
        self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 乐斗驿站(self):
        EventsTwo.get('cmd=newAct&subtype=167&op=2')
        self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 开心娃娃机(self):
        EventsTwo.get('cmd=newAct&subtype=124&op=1')
        self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 好礼步步升(self):
        EventsTwo.get('cmd=newAct&subtype=43&op=get')
        self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 浩劫宝箱(self):
        EventsTwo.get('cmd=newAct&subtype=152')
    ...
    def 幸运转盘(self):
        EventsTwo.get('cmd=newAct&subtype=57&op=roll')
        self.msg.append(DaLeDou.search(r'0<br /><br />(.*?)<br />'))

    def 惊喜刮刮卡(self):
        for i in range(3):
            EventsTwo.get(f'cmd=newAct&subtype=148&op=2&id={i}')
            self.msg.append(DaLeDou.search(r'奖池预览</a><br /><br />(.*?)<br />'))

    def 甜蜜夫妻(self):
        for i in range(1, 4):
            # 领取
            EventsTwo.get(f'cmd=newAct&subtype=129&op=1&flag={i}')

    def 乐斗菜单(self):
        # 乐斗菜单
        EventsTwo.get('cmd=menuact')
        if gift := DaLeDou.findall(r'套餐.*?gift=(\d+).*?点单</a>'):
            # 点单
            EventsTwo.get(f'cmd=menuact&sub=1&gift={gift[0]}')
    ...
    def 客栈同福(self):
        for _ in range(3):
            # 献酒
            EventsTwo.get('cmd=newAct&subtype=155')
    ...
    def 周周礼包(self):
        # 周周礼包
        EventsTwo.get('cmd=weekgiftbag&sub=0')
        if id := DaLeDou.findall(r';id=(\d+)">领取'):
            # 点单
            EventsTwo.get(f'cmd=weekgiftbag&sub=1&id={id[0]}')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 登录有礼(self):
        # 登录有礼
        EventsTwo.get('cmd=newAct&subtype=56')
        # 领取
    ...
    def 活跃礼包(self):
        for p in ['1', '2']:
            EventsTwo.get(f'cmd=newAct&subtype=94&op={p}')
            self.msg.append(DaLeDou.search(r'】.*?<br />(.*?)<br />'))

    def 猜单双(self):
        # 猜单双
        EventsTwo.get('cmd=oddeven')
        for _ in range(5):
            if value := DaLeDou.findall(r'value=(\d+)">.*?数'):
                value = choice(value)
                # 单数1 双数2
                EventsTwo.get(f'cmd=oddeven&value={value}')
                self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
            else:
                break
    ...
    def 上香活动(self):
        for _ in range(2):
            # 檀木香
            EventsTwo.get('cmd=newAct&subtype=142&op=1&id=1')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
            # 龙涎香
            EventsTwo.get('cmd=newAct&subtype=142&op=1&id=2')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 徽章战令(self):
        # 每日礼包
        EventsTwo.get('cmd=badge&op=1')
        self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 生肖福卡(self):
        # 领取
        EventsTwo.get('cmd=newAct&subtype=174&op=7&task_id=1')
        self.msg.append(DaLeDou.search(r'~<br /><br />(.*?)<br />活跃度80'))

    def 长安盛会(self):
        for id in ['1', '2']:
            # 【盛会豪礼】》【签到宝箱】
            EventsTwo.get(f'cmd=newAct&subtype=118&op=1&id={id}')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
        for _ in range(3):
            # 点击参与
            EventsTwo.get('cmd=newAct&subtype=118&op=1&id=5')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
            if '剩余次数不足' in html:
                break
    ...
    def 深渊秘宝(self):
        ...
        '''
        # 深渊秘宝
        EventsTwo.get('cmd=newAct&subtype=175')
    ...
    def 盛世巡礼(self):
        ...
        for itemId in [0, 4, 6, 9, 11, 14, 17]:
            # 收下礼物
            EventsTwo.get(f'cmd=newAct&subtype=150&op=5&itemId={itemId}')
            self.msg.append(DaLeDou.search(r'礼物<br />(.*?)<br />'))

    def 十二周年生日祝福(self):
        for day in range(1, 8):
            EventsTwo.get(f'cmd=newAct&subtype=165&op=3&day={day}')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 中秋礼盒(self):
        for _ in range(3):
            # 中秋礼盒
            EventsTwo.get('cmd=midautumngiftbag&sub=0')
            ...
            for id in ids:
                # 领取
                EventsTwo.get(f'cmd=midautumngiftbag&sub=1&id={id}')
                self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
    ...
    def 企鹅吉利兑(self):
        ...
        '''
        # 企鹅吉利兑
        EventsTwo.get('cmd=geelyexchange')

        # 修炼任务 》每日任务
        for id in DaLeDou.findall(r'id=(\d+)">领取</a>'):
            # 领取
            EventsTwo.get(f'cmd=geelyexchange&op=GetTaskReward&id={id}')
            self.msg.append(DaLeDou.search(r'】<br /><br />(.*?)<br /><br />'))
    ...
    def 乐斗游记(self):
        # 乐斗游记
        EventsTwo.get('cmd=newAct&subtype=176')

        # 今日游记任务
        for id in DaLeDou.findall(r'task_id=(\d+)'):
            # 领取
            EventsTwo.get(f'cmd=newAct&subtype=176&op=1&task_id={id}')
            self.msg.append(DaLeDou.search(r'积分。<br /><br />(.*?)<br />'))

        if self.week == '4':
            # 一键领取
            EventsTwo.get('cmd=newAct&subtype=176&op=5')
            self.msg.append(DaLeDou.search(r'积分。<br /><br />(.*?)<br />'))
            ...
            if (num := int(num_list[0])) != 0:
                ...
                for _ in range(num10):
                    # 兑换十次
                    EventsTwo.get('cmd=newAct&subtype=176&op=2&num=10')
                    self.msg.append(DaLeDou.search(
                        r'积分。<br /><br />(.*?)<br />'))
                for _ in range(num1):
                    # 兑换一次
                    EventsTwo.get('cmd=newAct&subtype=176&op=2&num=1')
                    self.msg.append(DaLeDou.search(
                        r'积分。<br /><br />(.*?)<br />'))

    def 万圣节(self):
        # 点亮南瓜灯
        EventsTwo.get('cmd=hallowmas&gb_id=1')
        while True:
            if cushaw_id := DaLeDou.findall(r'cushaw_id=(\d+)'):
                id = choice(cushaw_id)
                # 南瓜
                EventsTwo.get(f'cmd=hallowmas&gb_id=4&cushaw_id={id}')
                self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
            # 恭喜您获得10体力和南瓜灯一个！
        ...
        # 兑换奖励
        EventsTwo.get('cmd=hallowmas&gb_id=0')
        ...
        if int(self.date) == int(date) - 1:
            ...
            for _ in range(int(B)):
                # 礼包B 消耗40个南瓜灯
                EventsTwo.get('cmd=hallowmas&gb_id=6')
                self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
            for _ in range(int(A)):
                # 礼包A 消耗20个南瓜灯
                EventsTwo.get('cmd=hallowmas&gb_id=5')
                self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 双节签到(self):
        # 双节签到
        EventsTwo.get('cmd=newAct&subtype=144')
        date: str = DaLeDou.findall(r'至\d+月(\d+)日')[0]
        if 'op=1' in html:
            # 领取
            EventsTwo.get('cmd=newAct&subtype=144&op=1')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
        if int(self.date) == int(date) - 1:
            # 奖励金
            EventsTwo.get('cmd=newAct&subtype=144&op=3')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 圣诞有礼(self):
        # 圣诞有礼
        EventsTwo.get('cmd=newAct&subtype=145')
        for id in DaLeDou.findall(r'task_id=(\d+)'):
            # 任务描述：领取奖励
            EventsTwo.get(f'cmd=newAct&subtype=145&op=1&task_id={id}')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
        # 连线奖励
        for index in DaLeDou.findall(r'index=(\d+)'):
            EventsTwo.get(f'cmd=newAct&subtype=145&op=2&index={index}')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 乐斗回忆录(self):
        for id in [1, 3, 5, 7, 9]:
            # 回忆礼包
            EventsTwo.get(f'cmd=newAct&subtype=171&op=3&id={id}')
            self.msg.append(DaLeDou.search(r'6点<br />(.*?)<br />'))

    def 新春礼包(self):
        # 新春礼包
        EventsTwo.get('cmd=xinChunGift&subtype=1')
        date_list = DaLeDou.findall(r'~\d+月(\d+)日')
        giftid = DaLeDou.findall(r'giftid=(\d+)')
        for date, id in zip(date_list, giftid):
            if int(self.date) == int(date) - 1:
                EventsTwo.get(f'cmd=xinChunGift&subtype=2&giftid={id}')
                self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))
    ...
    def 新春登录礼(self):
        # 新春登录礼
        EventsTwo.get('cmd=newAct&subtype=99&op=0')
        for day in DaLeDou.findall(r'day=(\d+)'):
            # 领取
            EventsTwo.get(f'cmd=newAct&subtype=99&op=1&day={day}')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 喜从天降(self):
        # 点燃烟花
        EventsTwo.get('cmd=newAct&subtype=137&op=1')
        self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 春联大赛(self):
        # 开始答题
        EventsTwo.get('cmd=newAct&subtype=146&op=1')
        if '您的活跃度不足' in html:
        ...
        for _ in range(3):
            for s in DaLeDou.findall(r'上联：(.*?) 下联：'):
                ...
                if xialian:
                    ...
                    # 确定选择
                    EventsTwo.get('cmd=newAct&subtype=146&op=2')
                    self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

        for id in range(1, 4):
            # 领取
            EventsTwo.get(f'cmd=newAct&subtype=146&op=4&id={id}')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 年兽大作战(self):
        # 年兽大作战
        EventsTwo.get('cmd=newAct&subtype=170&op=0')
        if '等级不够' in html:
            self.msg.append('等级不够，还未开启年兽大作战哦！')
            return
        for _ in DaLeDou.findall(r'剩余免费随机次数：(\d+)'):
            # 随机武技库 免费一次
            EventsTwo.get('cmd=newAct&subtype=170&op=6')
        ...
        if '暂未选择' in html:
            for t in range(5):
                EventsTwo.get(f'cmd=newAct&subtype=170&op=4&type={t}')
                if '取消选择' in html:
                    continue
        ...
        for _ in range(3):
            # 挑战
            EventsTwo.get('cmd=newAct&subtype=170&op=8')
            self.msg.append(DaLeDou.search(r'帮助</a><br />(.*?)。'))

    def 冰雪企缘(self):
        # 冰雪企缘
        EventsTwo.get('cmd=newAct&subtype=158&op=0')
        for t in DaLeDou.findall(r'gift_type=(\d+)'):
            # 领取
            EventsTwo.get(f'cmd=newAct&subtype=158&op=2&gift_type={t}')
            self.msg.append(DaLeDou.search(r'】<br />(.*?)<br />'))

    def 煮元宵(self):
        # 煮元宵
        EventsTwo.get('cmd=yuanxiao2014')
        # number: list = DaLeDou.findall(r'今日剩余烹饪次数：(\d+)')
        for _ in range(4):
            # 开始烹饪
            EventsTwo.get('cmd=yuanxiao2014&op=1')
            if '领取烹饪次数' in html:
            ...
            for _ in range(20):
                ...
                if int(maturity[0]) >= 96:
                    # 赶紧出锅
                    EventsTwo.get('cmd=yuanxiao2014&op=3')
                    self.msg.append(DaLeDou.search(
                        r'活动规则</a><br /><br />(.*?)。'))
                    break
                # 继续加柴
                EventsTwo.get('cmd=yuanxiao2014&op=2')

    def 元宵节(self):
        # 领取
        EventsTwo.get('cmd=newAct&subtype=101&op=1')
        self.msg.append(DaLeDou.search(r'】</p>(.*?)<br />'))
        # 领取月桂兔
        EventsTwo.get('cmd=newAct&subtype=101&op=2&index=0')
        self.msg.append(DaLeDou.search(r'】</p>(.*?)<br />'))
