...
class AoYi(DaLeDou):
    ...
    def bangpaijitan(self) -> int:
        ...
        jifen: list = DaLeDou.findall(r'我的祭祀积分：(\d+)')
    ...
    def run(self) -> list:
        if self.祝福合集宝库():
            self.msg += DaLeDou.conversion('奥义')
            self.msg += ['升级技能奥义所需奥秘元素数量']
            calculate_number, practical_number, zhufuzhi_step = self.calculate()
            self.msg += [
                f'需要：{calculate_number}',
                f'拥有（商店+背包）：{practical_number}',
                f'五阶5星(含)前失败祝福值：{zhufuzhi_step}'
            ]
