...
class MissionPlanner:
    ...
    @staticmethod
    def get_available_missions() -> dict[str, list]:
        """获取可用任务"""
        ...
        return {
            "one": [k for k, v in one_missions.items() if v],
            "two": [k for k, v in two_missions.items() if v],
        }
...
def get_func_names(qq: str, session: Session) -> dict[str, list[str]]:
    ...
    missions = MissionPlanner.get_available_missions()
    # 获取并映射任务名称
    one_missions = [m for m in missions["one"] if m in html]
    two_missions = [m for m in missions["two"] if m in html]
