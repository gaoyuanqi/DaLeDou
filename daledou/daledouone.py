...
class DaLeDouOne(DaLeDou):
    ...
    def 门派(self):
        ...
        for op in ['fumigatefreeincense', 'fumigatepaidincense']:
            DaLeDouOne.get(f'cmd=sect&op={op}')
        ...
        if '查看一名' in wuhuatang:
            ...
            for page in [2, 3]:
                # 好友第2、3页
                DaLeDouOne.get(f'cmd=friendlist&page={page}')
                uin_list = DaLeDou.findall(r'\d+：.*?B_UID=(\d+).*?级')
                for uin in uin_list:
                    # 查看好友
                    DaLeDouOne.get(f'cmd=totalinfo&B_UID={uin}')
        ...
        if '进行一次心法修炼' in wuhuatang:
            ...
            for id in range(101, 119):
                DaLeDouOne.get(f'cmd=sect_art&subtype=2&art_id={id}&times=1')
                if '修炼成功' in html:
        ...
        for id in text_list:
            # 完成
            DaLeDouOne.get(f'cmd=sect_task&subtype=2&task_id={id}')
    ...
    def 我的帮派(self):
        ...
        # 帮派任务
        DaLeDouOne.get('cmd=factiontask&sub=1')
        ...
        if self.week == '0':
            for sub in [4, 9, 6]:
                # 领取奖励 》报名帮派战争 》激活祝福
                DaLeDouOne.get(f'cmd=facwar&sub={sub}')
                self.msg += DaLeDou.findall(r'</p>(.*?)<br /><a.*?查看上届')

        for _ in range(5):
            # 供奉守护神
            DaLeDouOne.get('cmd=viewolbation')
            text_list = DaLeDou.findall(r'数量：.*?id=(\d+).*?供奉</a>')
            # 供奉
            DaLeDouOne.get(f'cmd=oblation&id={text_list[0]}&page=1')
            self.msg += DaLeDou.findall(r'【供奉守护神】<br />(.*?)<br />')

        # 帮派任务
        DaLeDouOne.get('cmd=factiontask&sub=1')
        text_list = DaLeDou.findall(r'id=(\d+)">领取奖励</a>')
        for id in text_list:
            # 领取奖励
            DaLeDouOne.get(f'cmd=factiontask&sub=3&id={id}')
    ...
    def 大侠回归三重好礼(self):
        ...
        DaLeDouOne.get('cmd=newAct&subtype=173&op=1')
        text_list = DaLeDou.findall(r'天.*?taskid=(\d+)">领取')
        for id in text_list:
            # 领取
            DaLeDouOne.get(f'cmd=newAct&subtype=173&op=2&taskid={id}')
