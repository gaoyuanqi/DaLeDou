...
def get(params: str) -> str:
    ...
    Args:
        params: 大乐斗URL中cmd开头的链接，比如 cmd=xxx&xx=xx
...
def find(mode: str, name: str | None = None) -> str | None:
    '''
    查找大乐斗HTML字符串源码中成功匹配的第一个结果，失败则返回None
    匹配结果会被打印并写入日志
...
def 梦想之旅():
    ...
    MSG.append(find(r'规则</a><br />(.*?)<br />', '梦想之旅-普通旅行'))

    if WEEK == 4:
        if (count := HTML.count('已去过')) < 7:
            msg = f'已去过 {count} （大于等于7才会消耗梦幻机票）'
            info(msg)
            MSG.append(msg)
            return
        ...
        for name in place:
            ...
            MSG.append(find(r'规则</a><br />(.*?)<br />', '梦想之旅-梦幻旅行'))
        ...
        for _ in range(2):
            if b := findall(r'sub=4&amp;bmapid=(\d+)'):
                ...
                MSG.append(find(r'规则</a><br />(.*?)<br />', '梦想之旅-领取礼包'))
